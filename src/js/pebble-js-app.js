var setPebbleToken = 'T7M6';
var locationOptions = { "timeout": 15000, "maximumAge": 60000 };

function appMessageAck(e) {
    console.log("Message to Pebble sent successfully");
	//Pebble.showSimpleNotificationOnPebble("YES", "Message to pebble sent");
}

function appMessageNack(e) {
    console.log("Failed to send message to Pebble: " + e.error.message);
...
function getWeatherIcon(weather_id, sunset, sunrise){
	console.log("Weather ID: " + weather_id);
	var cdate = (new Date()).getTime()/1000;
	var is_day = (cdate < sunset && cdate > sunrise)? true : false;
	var icon = ")";
	
	if(weather_id >= 200){ icon = "P"; }
	if(weather_id >= 202){ icon = "0"; }
	if(weather_id >= 210){ icon = "P"; }
	if(weather_id >= 212){ icon = "0"; }
	if(weather_id >= 230){ icon = "P"; }
	if(weather_id >= 300){ icon = "Q"; }
	if(weather_id >= 502){ icon = "R"; }
	if(weather_id >= 520){ icon = "Q"; }
	if(weather_id >= 522){ icon = "R"; }
	if(weather_id >= 600){ icon = "V"; }
	if(weather_id >= 601){ icon = "U"; }
	if(weather_id >= 602){ icon = "W"; }
	if(weather_id >= 621){ icon = "V"; }
	if(weather_id >= 701){ icon = "M"; }
	if(weather_id >= 711){ icon = (is_day)? "J" : "K"; }
	if(weather_id >= 731){ icon = "F"; }
	if(weather_id >= 741){ icon = "M"; }
	if(weather_id >= 800){ icon = (is_day)? "B" : "C"; }
	if(weather_id >= 801){ icon = (is_day)? "H" : "I"; }
	if(weather_id >= 803){ icon = "N"; }
	if(weather_id >= 804){ icon = "Y"; }
	if(weather_id >= 900){ icon = "9"; }
	if(weather_id >= 903){ icon = "G"; }
	if(weather_id >= 904){ icon = "'"; }
	if(weather_id >= 905){ icon = "F"; }
	if(weather_id >= 906){ icon = "X"; }
	if(weather_id >= 950){ icon = "F"; }
	
	return icon;	
}

function sendWeatherOK(weather_response){
	console.log("Sending weather info to Pebble: " + JSON.stringify(weather_response));
	Pebble.sendAppMessage(weather_response, appMessageAck, appMessageNack);
}

function sendWeatherFail(code){
	console.log("Sending weather fail message to Pebble");
	Pebble.sendAppMesage({"action":code}, appMessageAck, appMessageNack);
}
/*
function fetchWeather(latitude, longitude){
	var weather_response = {"action":1,"temp":27600,"city":"Kosice","icon":"Q","desc":"Rain"};
	sendWeatherOK(weather_response);
}
*/
...
function fetchWeather(latitude, longitude){
	...
	//var url = 'http://api.openweathermap.org/data/2.5/weather?lat=48.63&lon=21.19';
		...
		if(request.readyState == 4 && request.status == 200){
			...
			if(response && response.main && response.weather){
				var weather_response = {
					"action": 1,
					"city": removeDiacritics(response.name),
					"temp": Math.round(100*response.main.temp),
				...
				console.log("Error within received data");
				sendWeatherFail(2);
			}
		} else {
			console.log("No response from weather");
			sendWeatherFail(2);
		}
	};
	request.send(null);
...
function locationSuccess(pos) {
	...
	console.log("GPS coordinates found: lat=" + coordinates.latitude + " lon=" + coordinates.longitude);
	//Pebble.showSimpleNotificationOnPebble("GPS coords: ", "lat=" + coordinates.latitude + "\nlon=" + coordinates.longitude);
...
Pebble.addEventListener('ready', function(e) {
	...
	Pebble.sendAppMessage({"action":3}, appMessageAck, appMessageNack);
